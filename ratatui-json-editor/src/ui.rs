...
pub fn ui(frame: &mut Frame, state: &State) {
    ...
    for key in state.pairs.keys() {
        list_items.push(ListItem::new(Line::from(Span::styled(
            format!("{: <25} : {}", key, state.pairs.get(key).unwrap()),
    ...
    if let Some(editing) = &state.currently_editing {
        ...
        let key_text = Paragraph::new(state.key_input.clone()).block(key_block);
        frame.render_widget(key_text, popup_chunks[0]);

        let value_text = Paragraph::new(state.value_input.clone()).block(value_block);
